...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        strategy = get_strategy(who)
        num_rolls = strategy(get_score(who), get_score(1-who))
        turn = take_turn(num_rolls, get_score(who), get_score(1-who), dice=dice, goal=goal)
        if who == 0:
            score0 += turn
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if leader is None or leader == last_leader:
        ...
        message = f"Player {leader} takes the lead by {abs(score1-score0)}"
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    turn = hefty_hogs(score, opponent_score)
    if hefty_hogs(score, opponent_score) >= threshold:
        return 0
    elif hog_pile(score+turn, opponent_score):
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
